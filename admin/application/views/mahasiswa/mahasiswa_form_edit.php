			...
			<form class="form" enctype="multipart/form-data" action="" method="post">
				<div class="box-body">
					...
					<div class="form-group row <?= form_error('email') ? 'has-error' : null ?>">
						...
						<div class="col-sm-8">
							<input type="email" class="form-control" name="email" placeholder="Email" value="<?= $mhs_id['email'] ?>">
							<?= form_error('email') ?>
						</div>
					</div>
							...
							<select class="form-control" name="dosen_pa">
								<?php
									if(!empty($dpa))
									{
										...
										if ($dpa_id == $mhs_id['dpa_id']) 
										{ 
								?>
											<option value="<?= $dpa_id ?>" selected><?= $nama_lkp ?></option>
								<?php			
										}
										else 
										{
